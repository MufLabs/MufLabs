export class WorkflowEngine {
    ...
    async execute(context, request) {
        ...
        console.log("Constructor:", agentManager?.constructor?.name);
        console.log("resolveByCommand:", typeof agentManager?.resolveByCommand);
        console.log("Manager State:", agentManager?.getState?.());
        console.log("Registry Size:", agentManager?.getRegistry?.().size);
        try {
            ...
            if (registry) {
                ...
                console.table(agents.map((agent) => ({
                    id: agent.id,
                    name: agent.name,
                    capabilities: agent.definition?.capabilities?.map((c) => typeof c === "string"
                        ? c
                        : c.name) ?? []
                })));
                console.log("===============================");
            ...
            console.error("Unable to inspect registry:", error);
        }
        ...
        console.log("Resolved Agent:", agent?.name ?? "<NOT FOUND>");
        if (!agent) {
